class View {
  ...
  constructor(WEEKDAYS, MEALS) {
    ...
    this.#modal_update_button = document.getElementById(
  ...
  bindUpdateSchedule(callback) {
    ...
    this.#modal_update_button.addEventListener(
      "click",
  ...
  #initCommonListeners() {
    this.#modal_close_button.addEventListener(
      "click",
  ...
  #updateSchedule() {
    if (this.#foodTextbox.value !== null) {
      const newFoodItem = this.#foodTextbox.value
        .split(";")
      ...
      this.#updateScheduleCallback(
        this.#daySelect.value.toLowerCase(),
        this.#mealSelect.value.toLowerCase(),
        newFoodItem
      );
    }
  ...
  updateHTML(data, days = this.#WEEKDAYS, meals = this.#MEALS) {
    for (const day of days) {
      for (const meal of meals) {
        const ulLiveList = document.querySelector(
          "#" + day + "-schedule" + " ." + meal + "-meal-div ul"
        );
        ulLiveList.parentNode.replaceChild(
          this.#convertToULTag(data[day][meal], ulLiveList),
          ulLiveList
        );
      }
    }
