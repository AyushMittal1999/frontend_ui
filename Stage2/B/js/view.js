function View(wdays, meals) {
  ...
  const modal_update_button = document.getElementById(
    "modal-update-changes-button"
  );
  ...
  function createDayLayout(day) {
    ...
    MEALS.slice(0, 2).forEach((m) =>
      two_card_group1.appendChild(createMealLayout(day, m))
    );

    const two_card_group2 = document.createElement("div");
    two_card_group2.className = "day-schedule__two-card-group";
    MEALS.slice(2, 4).forEach((m) =>
      two_card_group2.appendChild(createMealLayout(day, m))
    );
  ...
  this.bindUpdateSchedule = function (callback) {
  ...
  function initCommonListeners() {
    modal_close_button.addEventListener("click", this.hideModal);

    //Listener to display Modal
    edit_button.addEventListener("click", this.showModal);
  }

  initCommonListeners.call(this);

  function updateSchedule() {
    if (foodTextbox.value !== null) {
      const newFoodItem = foodTextbox.value
        .split(";")
  ...
  this.displayStatus = function (isSuccess) {
  ...
  function updateHTMLToday(data) {
    ...
    today_breakfast.textContent =
      newitems.breakfast.length > 0 ? newitems.breakfast.join(", ") : "nothing";
    today_lunch.textContent =
      newitems.lunch.length > 0 ? newitems.lunch.join(", ") : "nothing";
    today_snacks.textContent =
      newitems.snacks.length > 0 ? newitems.snacks.join(", ") : "nothing";
    today_dinner.textContent =
      newitems.dinner.length > 0 ? newitems.dinner.join(", ") : "nothing";
  }

  // Update Cards on HTML for specified days
  this.updateHTML = function (data, days = WEEKDAYS, meals = MEALS) {
    for (const day of days) {
      for (const meal of meals) {
        const ulLiveTag = document.querySelector(
          "#" + day + "-" + meal + "-card" + " ul"
        );
        const fragment = new DocumentFragment();
