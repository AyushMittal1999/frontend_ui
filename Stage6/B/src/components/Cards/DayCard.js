...
const DayCard = memo(function DayCard({ day, meal }) {
  ...
  return (
    <Card
      className="flex flex-1 flex-row h-105 min-w-100 xs:min-w-11/12 m-4.5 rounded-4xl focus:h-110 focus:outline-none"
    ...
    >
      <CardContent className="w-full">
        ...
        >
          ...
          <Avatar className="bg-white -mb-6 h-16 w-18">
            {index === 0 ? (
        ...
        <Zoom in={true} timeout={1200} style={{ transitionDelay: "300ms" }}>
          ...
          <LazyChipWrapper>
            <Chip
              ...
              icon={<ScheduleIcon />}
              label={TIME[index]}
              className="text-lime-600 border-lime-600 ml-1"
            />
          </LazyChipWrapper>
        </Zoom>
