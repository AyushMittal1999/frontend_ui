const Weekday = React.memo(
  function Weekday({ day, dataService, specificMealUpdate }) {
            ...
            {meals.slice(0, 2).map((m) => {
              return (
                <Card
                  ...
                  imgAttributes={{
                    src: `../resources/images/${m}.jpg`,
                    alt: m,
                  }}
                  data={dataService.getMealData(day, m)}
                  rerender={
                    specificMealUpdate && specificMealUpdate != m ? 0 : 1
                  }
                />
              ); // Skip card rerendering if some other meal is updated
            })}
          </div>
